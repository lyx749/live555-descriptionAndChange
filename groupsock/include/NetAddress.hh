...
public:
  NetAddress(u_int8_t const* data,
	     unsigned length = 4 /* default: 32 bits */);//根据data的数据结构构造地址
  NetAddress(unsigned length = 4); // sets address data to all-zeros；构造一个0.0.0.0 的ip地址
  NetAddress(NetAddress const& orig); //根据另外一个对象构建地址
  NetAddress& operator=(NetAddress const& rightSide);//赋值构造
  virtual ~NetAddress();
