      ...
      fAppHandlerTask(NULL), fAppHandlerClientData(NULL)
{
  ...
  {
    /**
     * 如果设置了复用RTCP和RTP，则执行以下操作：fInitialPortNum = initialPortNum：将初始端口号设置为指定的端口号。
     */
  ...
  {
    /**
     * 如果未设置复用RTCP和RTP，则执行以下操作：fInitialPortNum = (initialPortNum+1)&~1：将初始端口号设置为指定端口号加1，并进行按位与操作，以确保RTP端口号为偶数。
     */
    // Make sure RTP ports are even-numbered:
    fInitialPortNum = (initialPortNum + 1) & ~1;
  }
  /**
   * 获取主机名，并将其存储在成员变量fCNAME中。
   */
...
OnDemandServerMediaSubsession::sdpLines()
{
  if (fSDPLines == NULL)
  {
    ...
    /**
     *estBitrate用于存储估计的比特率值。比特率是指单位时间内传输的数据量，通常以每秒传输的比特（bits per second）为单位。
     *在流媒体中，比特率表示每秒传输的媒体数据的位数。
     *在生成SDP行的过程中，通过创建虚拟的RTPSink对象，并调用其estimatedBitrate()方法来获取估计的比特率值。
     *这个估计的比特率值可以用于在SDP行中提供关于媒体流的传输特性和要求的信息。
     *估计的比特率在流媒体传输中具有重要的作用，它可以帮助客户端设备和网络进行合理的带宽分配和调整。
     *通过提供准确的比特率信息，接收端可以根据其网络带宽和处理能力做出相应的处理，以确保流媒体的顺畅播放和最佳性能。
     *因此，estBitrate变量的作用是存储估计的比特率值，以在生成SDP行时提供媒体流的传输特性和要求的信息。
     */
    unsigned estBitrate;

    // 打开该session的文件源，并且生成该子session的解释器
...
float OnDemandServerMediaSubsession::getCurrentNPT(void *streamToken)
{
  do
  {
    ...
    return streamState->startNPT() + (rtpSink->mostRecentPresentationTime().tv_sec - rtpSink->initialPresentationTime().tv_sec) + (rtpSink->mostRecentPresentationTime().tv_usec - rtpSink->initialPresentationTime().tv_usec) / 1000000.0f;
  } while (0);
...
void OnDemandServerMediaSubsession ::getRTPSinkandRTCP(void *streamToken,
...
void OnDemandServerMediaSubsession ::setStreamSourceDuration(FramedSource * /*inputSource*/, double /*streamDuration*/, u_int64_t &numBytes)
{
  // Default implementation: Do nothing
  numBytes = 0;
}
...
Groupsock *OnDemandServerMediaSubsession ::createGroupsock(struct in_addr const &addr, Port port)
{
  // Default implementation; may be redefined by subclasses:
  return new Groupsock(envir(), addr, port, 255);
}

RTCPInstance *OnDemandServerMediaSubsession ::createRTCP(Groupsock *RTCPgs, unsigned totSessionBW, /* in kbps */
...
void OnDemandServerMediaSubsession ::setRTCPAppPacketHandler(RTCPAppHandlerFunc *handler, void *clientData)
{
  fAppHandlerTask = handler;
  fAppHandlerClientData = clientData;
}

void OnDemandServerMediaSubsession ::sendRTCPAppPacket(u_int8_t subtype, char const *name,
...
void OnDemandServerMediaSubsession ::setSDPLinesFromRTPSink(RTPSink *rtpSink, FramedSource *inputSource, unsigned estBitrate)
{
  if (rtpSink == NULL)
    return;
  ...
  unsigned sdpFmtSize = strlen(sdpFmt) + strlen(mediaType) + 5 /* max short len */ + 3 /* max char len */
                        ...
                        + strlen(rtpmapLine) + strlen(rtcpmuxLine) + strlen(rangeLine) + strlen(auxSDPLine) + strlen(trackId());
...
void StreamState::endPlaying(Destinations *dests, unsigned clientSessionId)
{
  ...
  else
  {
    ...
    if (fRTPgs != NULL)
      fRTPgs->removeDestination(clientSessionId);
