...
/// @brief 调用Media.hh中的_Tables::getOurTables方法初始化其成员变量socketTable
...
/**
 * RTPInterface类中保存了组播地址，和tcp流的地址和通道id，然后每一个tcp套接字有多个通道，这个通道管理就是由对应的SocketDescriptor
 * 类来完成的，这个类的管理定义在Media.hh下，是一个hashTable，以套接字描述符作为key，然后这个SocketDescriptor下有一个管理对应套接字
 * 的子通道的hashTable，以通道id作为key，该tcp套接字所在的RTPInterface为value
*/

/// @brief 套接字描述符(Socket Descriptor)的封装类。它用于储TCP流记录的相关信息管理套接字的操作和状态
...
public:
  ...
  /// @brief 注册一个RTP接口对象到指定的流通道ID上
  /// @param streamChannelId 指定的流通道ID
  /// @param rtpInterface RTP接口对象
  void registerRTPInterface(unsigned char streamChannelId,
                            RTPInterface *rtpInterface);

  /// @brief 根据streamChannelId从fSubChannelHashTable中查找对应的RTPInterface
  /// @param streamChannelId 需要查找的流通道id
  /// @return RTPInterface*类指针
  RTPInterface *lookupRTPInterface(unsigned char streamChannelId);

  /// @brief 取消注册指定流通道ID上的RTP接口对象
...
private:
  ...
  /**
   * 这两个函数用于处理TCP读取事件的回调函数
  */
  static void tcpReadHandler(SocketDescriptor *, int mask);
  Boolean tcpReadHandler1(int mask);

private:
  UsageEnvironment &fEnv;                                                    // UsageEnvironment对象的引用，用于提供运行环境和相关功能
  int fOurSocketNum;                                                         // TCP流的套接字号，用于标识与远程主机之间的连接
  HashTable *fSubChannelHashTable;                                           // 指向HashTable对象的指针，用于存储子通道的信息
  ServerRequestAlternativeByteHandler *fServerRequestAlternativeByteHandler; // 指向ServerRequestAlternativeByteHandler对象的指针，用于处理服务器请求替代字节的操作
  void *fServerRequestAlternativeByteHandlerClientData;                      // 指向服务器请求替代字节处理器的客户端数据的指针
  u_int8_t fStreamChannelId, fSizeByte1;                                     // 流通道的ID，用于标识不同的流通道，用于存储数据包的大小
  Boolean fReadErrorOccurred;                                                // 标志位，指示是否发生了读取错误
  Boolean fDeleteMyselfNext;                                                 // 标志位，指示是否在下一次循环中删除自身
  Boolean fAreInReadHandlerLoop;                                             // 标志位，指示是否正在读取处理循环中
  enum
  {
    AWAITING_DOLLAR,
  ...
  } fTCPReadingState;   //TCP读取状态的枚举值，用于指示当前TCP读取的阶段
};
...
SocketDescriptor::~SocketDescriptor()
{
  ...
  if (fServerRequestAlternativeByteHandler != NULL)
  {
    ...
    /**
     * 0xFF：这是一种特殊的信令字符，通常用于表示发生了错误或异常情况。在上述代码中，
     * 如果在读取TCP套接字时出现错误，就会传递0xFF给回调函数 fServerRequestAlternativeByteHandler，以通知它发生了错误。
     * 0xFE：这是另一种特殊的信令字符，通常用于表示需要重新控制TCP套接字的情况。在上述代码中，
     * 如果没有发生错误，但回调函数 fServerRequestAlternativeByteHandler 需要重新控制TCP套接字继续处理后续数据，就会传递0xFE给回调函数，以通知它需要继续处理。
    */
...
Boolean SocketDescriptor::tcpReadHandler1(int mask)
{
  ...
  case AWAITING_DOLLAR:
  {
    ...
    /**
     * 当读取到的字符不是'$'时，说明该字符属于RTSP请求或命令的一部分，
     * 而不是RTP或RTCP数据包。首先，函数会检查是否设置了fServerRequestAlternativeByteHandler，
     * 该变量指向一个回调函数，用于处理RTSP请求或命令。如果没有设置该回调函数，表示不处理RTSP请求或命令，直接忽略当前字符。
     * 如果设置了fServerRequestAlternativeByteHandler，且读取到的字符不是特殊的信令字符（0xFF和0xFE），
     * 则调用该回调函数。这样可以将读取到的RTSP请求或命令交由该回调函数处理。注意，特殊信令字符通常用于特殊目的，
     * 不应该发送到回调函数中。回调函数的参数为fServerRequestAlternativeByteHandlerClientData和读取到的字符c，
     * 通过这两个参数，可以将读取到的RTSP请求或命令传递给回调函数进行处理。总之，这段代码用于处理从TCP流中读取到的非'$'字符，
     * 将其交由回调函数处理。回调函数可根据应用需求对RTSP请求或命令进行自定义处理。如果没有设置回调函数，
     * 则忽略读取到的RTSP请求或命令字符
    */
    else
    {
