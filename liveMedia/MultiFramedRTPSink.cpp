...
void MultiFramedRTPSink::setMarkerBit()
{
   ...
   */
  rtpHdr |= 0x00800000; // M标记位于第8个bit位，正好将M位置为1
...
{
  // RTP数据包的特殊头部可能包含多个字，每个字都是32位（4字节）长。fSpecialHeaderPosition是特殊头部在输出缓冲区（fOutBuf）中的起始位置
  // （以字节为单位），而wordPosition表示要插入的特殊头部字的位置索引。
  // 由于每个字是32位（4字节）长，所以如果要插入特殊头部的第一个字，则需要插入到fSpecialHeaderPosition处，
  // 即从起始位置开始插入。如果要插入特殊头部的第二个字，则需要插入到fSpecialHeaderPosition + 4处，以此类推。
...
{
  /**
   * 填充字节是为了确保RTP数据包的总长度达到指定的最大大小。
   * RTP报文中的填充字节不携带任何有效数据，只是用来填充数据包，以达到指定的大小。
   */
  if (numPaddingBytes > 0)
  {
    ...
    /**
     * 创建一个最大长度为255字节的填充缓冲区paddingBuffer，并将其中的所有字节设置为0。
     * 然后，将最后一个字节设置为numPaddingBytes的值，以表示填充字节的数量。
     */
    ...
    // P（1 bit）位于第3个bit位：用于RTP报文（packet）结束点的预留空间，视报文是否需要多余的填塞空间
    unsigned rtpHdr = fOutBuf->extractWord(0);
    rtpHdr |= 0x20000000;
    fOutBuf->insertWord(rtpHdr, 0);
  }
...
void MultiFramedRTPSink::packFrame()
{
  ...
  else
  {
    ...
    // 对于h264or5文件来说这里调用的是H264or5VideoRTPSink.cpp文件下定义的H264or5Fragmenter类内定义的doGetNextFrame()虚函数
...
void MultiFramedRTPSink ::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
                                             ...
                                             unsigned durationInMicroseconds)
{
  // 先检查是否为第一个数据包，如果是，则记录当前时间为下一次发送时间。
  if (fIsFirstPacket)
  {
    // Record the fact that we're starting to play now:
    gettimeofday(&fNextSendTime, NULL);
  }
  // 保存帧的呈现时间并更新初始呈现时间（如果尚未设置）
  ...
  unsigned numFrameBytesToUse = frameSize; // 当前帧率要使用的大小
  ...
  // 如果之前已经将一帧或多帧数据打包进当前数据包，检查这一帧是否可以打包在它们后面。
  // 这是独立于数据包是否有足够空间存放这一帧的检查，这个检查稍后进行。
  if (fNumFramesUsedSoFar > 0)
  {
    // 如果上一帧结束了分段并且不允许在最后一个分片后跟其他帧，或者这一帧不能在数据包起始位置后出现，则保存这一帧以便下次使用。
  ...
  if (numFrameBytesToUse > 0)
  {
    ...
    if (fOutBuf->wouldOverflow(frameSize))
    {
      ...
      // 如果不能立即使用这一帧，将其作为溢出数据保存，并在下一个数据包中发送。
      // 但如果这一帧的大小超过了一个数据包的大小，则需要对其进行分片，
      // 并在当前数据包中使用部分数据（如果负载格式允许）。
      if (isTooBigForAPacket(frameSize) && (fNumFramesUsedSoFar == 0 || allowFragmentationAfterStart()))
      {
        ...
        overflowBytes = computeOverflowForNewFrame(frameSize); // 计算溢出的数据量
  ...
  else
  {
    ...
    if (fOutBuf->isPreferredSize() || fOutBuf->wouldOverflow(numFrameBytesToUse) || (fPreviousFrameEndedFragmentation && !allowOtherFramesAfterLastFragment()) || !frameCanAppearAfterPacketStart(fOutBuf->curPtr() - frameSize, frameSize))
    {
...
void MultiFramedRTPSink::sendPacketIfNecessary()
{
  ...
  // 如果输出缓冲区fOutBuf中有溢出数据（overflowData），并且溢出数据量超过了缓冲区总大小的一半，则进行一个效率优化操作：
  // 将输出缓冲区的packetStart指针重新设置到溢出数据的位置，这样在构建下一个数据包时就无需执行memmove()操作将溢出数据移动到正确的位置。
  ...
  else
  {
    ...
    // 如果还有帧数据需要发送，则计算出下一帧数据的播放时间fNextSendTime，
    // 并根据播放时间进行延时，等待相应时间后再次调用sendNext()函数发送下一个数据包
