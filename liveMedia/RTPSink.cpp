...
u_int32_t RTPSink::presetNextTimestamp() {
  ...
  // 函数会检查是否已经有多个目的地（多个传输目标）。这意味着RTP数据包将被复制并发送到多个目标，通常用于多播或广播传输。
  // 如果已经有其他传输目标，就不调整时间戳流，并直接返回当前时间戳值。
  // 但是，如果只有一个目的地，即只有单播传输，就会将当前时间戳设为fTimestampBase，
  // 并将fNextTimestampHasBeenPreset标记设置为True，表示下一个时间戳已经预设好了。这样做的目的是为了在单播传输中维护一个连续的时间戳流，
  // 确保数据包的时间戳是递增的。
...
/**
 * 这个函数是用于生成SDP（Session Description Protocol）中的a=rtpmap:行的字符串表示。
 * SDP是一种用于描述会话参数的文本格式，通常用于描述实时媒体传输会话的相关信息，
 * 例如音频和视频的编码格式、媒体传输地址、端口等。在RTP（Real-time Transport Protocol）会话中，
 * a=rtpmap:行用于指定RTP数据包的有效负载格式和编码参数。该行的格式如下：
 * a=rtpmap:<payload_type> <encoding_name>/<clock_rate>[/<encoding_parameters>]
 * <payload_type>: 表示RTP数据包的有效负载类型，取值范围通常是0-127，其中96及以上被称为动态负载类型（Dynamic Payload Type），而0-95为静态负载类型（Static Payload Type）。
 * <encoding_name>: 表示编码格式的名称，例如"PCMU"表示G.711 μ-law音频编码，"H264"表示H.264视频编码等。
 * <clock_rate>: 表示RTP时钟频率，用于计算RTP时间戳的单位时间间隔。
 * <encoding_parameters> (可选): 表示编码参数，通常用于指定音频或视频流的附加属性，例如音频的通道数等。
*/
